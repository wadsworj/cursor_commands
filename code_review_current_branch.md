...
Perform a code review of the **current branch**, comparing it against its **parent branch** (`development` or `develop`).

Use the git CLI to inspect the differences against the parent branch:

```
git diff <parent-branch>...
```

Only review the diff between the current branch and its parent branch. Do not include changes that are unstaged or committed on other branches.
...
> This change on the current branch introduces X to support Y. It updates A, B, and C modules and adds new logic for Z. No changes to deployment or configuration.
